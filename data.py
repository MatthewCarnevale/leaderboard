...
def rankedPull():
    ...
    playerList = []
    fetchPlayers = "SELECT name,level,tier,rank,lp,mmr,lpdelta,dailygames,wins,losses FROM playerdata ORDER BY id DESC LIMIT 10"
...
def yesterdaysDelta():
    conn, cur = dbCon()
    sql = "SELECT yesterdaysdelta FROM dailylp ORDER BY id DESC LIMIT 10"
    cur.execute(sql)
